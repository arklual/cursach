...
- Name: intro-and-guide.spec.ts >> Workflow editor — Guide modal first-visit >> Guide modal auto-opens on first visit and persists "seen" on close
- Location: e2e/intro-and-guide.spec.ts:72:7
...
Locator: getByRole('heading', { name: 'Как пользоваться редактором' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: 'Как пользоваться редактором' })
