...
describe('ExecutionService', () => {
  ...
  describe('polling', () => {
    it('останавливается при статусе success и применяет финальный run', fakeAsync(() => {
      ...
      tick(800); // первый poll — running
      tick(800); // второй poll — success → complete
      flush();
    ...
    it('переживает транзиентные ошибки polling-tick без падения', fakeAsync(() => {
      ...
      svc.executeWorkflow('wf-1').subscribe();
      tick(800); // первый tick — ошибка, должна быть проглочена
      tick(800); // второй tick — успех, останавливаемся
      flush();
