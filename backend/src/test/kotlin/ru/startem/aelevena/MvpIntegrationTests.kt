...
import ru.startem.aelevena.run.NodeRunRepository
...
class MvpIntegrationTests {
    ...
    @Test
    fun testWebhookTriggerCreatesRun() {
        val created = workflowService.createWorkflow(ru.startem.aelevena.api.dto.WorkflowCreateRequest(name = "webhook-test-${UUID.randomUUID()}"))
        val workflowId = UUID.fromString(created.meta.id)
        val versionId = created.graph.versionId.toLong()
        workflowService.updateGraph(
            ...
            WorkflowGraph(
                versionId = versionId.toString(),
                nodes = listOf(Node(id = "wh", type = "trigger.webhook", position = Position(0.0, 0.0), data = NodeData(label = "WH"))),
                connections = emptyList(),
            ),
        )

        val token = triggerService.list(workflowId).first { it.type == "webhook" }.token!!

        val runId = triggerService.handleWebhook(token, objectMapper.readTree("""{"x":1,"name":"flux"}"""))
        val finished = waitUntilFinished(runId, Duration.ofSeconds(10))

        assertEquals("success", finished.status)
        assertEquals("webhook", finished.triggerType, "запуск должен быть помечен типом триггера webhook")

        val whRun = nodeRuns.listByWorkflowRun(runId).first { it.nodeId == "wh" }
        val output = objectMapper.readTree(whRun.outputJson)
        assertEquals(1, output.get("x").asInt(), "входное тело webhook'а должно быть передано в workflow")
        assertEquals("flux", output.get("name").asText())
    }
