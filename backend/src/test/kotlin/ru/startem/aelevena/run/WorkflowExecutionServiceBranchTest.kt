...
class WorkflowExecutionServiceBranchTest {
    ...
    /**
     * Test A: split-mode + Merge.
     * Graph: Split(strategy=random, seed=42, mode=split) → passA(dataflow.foreach) → Merge
     *                                                     → passB(dataflow.foreach) → Merge
     * Edges from Split are tagged variant=A and variant=B.
     * Input: 8 items [{"id":1}, ..., {"id":8}].
     * Assert: merge output is array of 8 items, every item has _variant, variants ⊆ {A,B}.
     */
    @Test
    fun `split-mode distributes all items and merge collects them with variant tags`() {
        ...
        // Все узлы должны выполниться успешно
        ...
        // Merge output — массив из 8 элементов
        ...
        // Каждый элемент имеет поле _variant ∈ {A, B}
        ...
        // При seed=42 оба варианта должны попасть в выборку из 8 элементов
        assertTrue(variants.isNotEmpty(), "Набор вариантов не должен быть пустым")
    }

    /**
     * Test B: pick-mode + skip.
     * Graph: Split(strategy=attribute, mode=pick, rule: country=='RU'→A, default→B) → passA → Merge
     *                                                                                 → passB → Merge
     * Input: [{"country":"RU"}] → chosen=A → passB должен быть skipped.
     * Assert: passA=success, passB=skipped, merge=success.
     * Merge output contains only items from branch A.
     */
    @Test
    fun `pick-mode skips non-chosen branch and merge succeeds with only active branch items`() {
        ...
        // passB — skipped (branch B не выбрана)
        ...
        // Merge output содержит только элементы из ветки A (1 элемент с country=RU)
        ...
        // Элементы из ветки B не должны присутствовать (там country!=RU не было)
        for (item in mergeOutput) {
            if (item.has("country")) {
