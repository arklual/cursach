...
/**
 * Полный сценарий пошаговой отладки через REST: start → step → runToEnd → close,
 * плюс негативные ветки (нет версии, нет сессии, узел не ready, провал ноды).
 * Покрывает DebugController, DebugSessionService и его inner Session, плюс
 * сериализацию всех Debug DTO.
 */
@Testcontainers
@SpringBootTest
...
class DebugSessionIntegrationTest {
    ...
    fun `full happy-path step-through`() {
        ...
        // readyInputs preview is non-empty
        assertTrue(started.get("readyInputs").has("a"))

        // Re-GET returns same session
        ...
        // Step without nodeId → executes first ready (b)
        mockMvc.perform(
            post("/debug-sessions/$sessionId/step")
                .contentType(MediaType.APPLICATION_JSON),
        ).andExpect(status().isOk)
        ...
        // runToEnd on already-done session is a no-op
        mockMvc.perform(post("/debug-sessions/$sessionId/run-to-end"))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.status").value("done"))

        // Step on done session also no-op
    ...
    fun `start with explicit startNodeId restricts reachable set`() {
        ...
        // Only b and c should be reachable; a is excluded.
    ...
    fun `step with non-ready nodeId returns 400`() {
        ...
        val sessionId = started.get("sessionId").asText()
        // 'c' is not ready while 'a' and 'b' are unfinished
        mockMvc.perform(
            post("/debug-sessions/$sessionId/step")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""{"nodeId":"c"}"""),
    ...
    fun `failing node marks session failed and ready becomes empty`() {
        ...
        // Step a → ok. Step x → blows up inside executor.
        mockMvc.perform(post("/debug-sessions/$sessionId/step"))
            .andExpect(status().isOk)
        ...
        // ready is empty once anything failed
