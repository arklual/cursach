...
/**
 * Покрывает error-paths и op-таблицу matches() — то, чего не было в основном
 * BranchSplitStrategiesTest. Большая часть JaCoCo-misses тут — это ветки
 * sealed when'а для op + require'ы.
 */
class BranchSplitStrategiesExtraTest {
    ...
    fun `attribute gt false when value not number`() {
        ...
        // Right-hand is not a number → comparison is false → default variant.
    ...
    fun `stratified empty stratum still deterministic by user`() {
        ...
        // stratum-by absent → empty string, but still consistent.
