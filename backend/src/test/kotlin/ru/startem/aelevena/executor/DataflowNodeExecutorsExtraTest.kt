...
/**
 * Покрывает edge-paths private-функций DataflowNodeExecutors.kt (resolveStreamInput,
 * compare с разными op'ами и нечисловыми value, asArrayOrEmpty с не-массивом, fieldOrSelf).
 */
class DataflowNodeExecutorsExtraTest {
    ...
    // -------- resolveStreamInput через filter --------
    ...
    fun `filter keeps envelope as-is when multiple upstreams and no from`() {
        ...
        // No way to disambiguate → envelope falls through to asArrayOrEmpty → wraps the envelope object itself
    ...
    fun `filter passes through when from key is missing`() {
        ...
        // from doesn't match → envelope falls through; only single upstream rule applies.
        // 1 upstream "x" → takes [1,2]
    ...
    fun `filter wraps non-array single item`() {
        ...
        // not array → wrapped into one-element list
        assertEquals(1, out.size())
    }

    // -------- compare op error paths --------
    ...
    fun `map identity when item is not object`() {
        ...
        // Non-object items: select can't apply → passes through.
        assertEquals(3, out.size())
    }
