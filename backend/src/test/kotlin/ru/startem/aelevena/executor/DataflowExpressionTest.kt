...
/**
 * Проверка dataflow-нод в режиме выражений item/acc (ПМИ 6.4): filter `item.age > 18`,
 * map `{name: item.name.toUpperCase()}`, reduce `acc + item.price`, foreach (passthrough),
...
class DataflowExpressionTest {
    ...
    fun `invalid expression type surfaces as failure`() {
        ...
        // ссылка на несуществующую переменную → ReferenceError при исполнении
