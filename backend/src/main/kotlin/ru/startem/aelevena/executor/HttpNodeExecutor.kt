...
class HttpNodeExecutor(
    ...
    private companion object {
        ...
        const val ERROR_BODY_SNIPPET_LENGTH = 500
    }
    ...
    override fun execute(nodeId: String, config: JsonNode?, input: JsonNode): JsonNode {
        ...
        val failOnErrorStatus = config?.get("failOnErrorStatus")?.asBoolean() ?: true
        if (failOnErrorStatus && response.statusCode() >= 400) {
            val snippet = rawBody.take(ERROR_BODY_SNIPPET_LENGTH)
            throw RuntimeException("HTTP $method $url returned status ${response.statusCode()}: $snippet")
        }
