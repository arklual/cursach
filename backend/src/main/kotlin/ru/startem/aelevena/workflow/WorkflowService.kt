...
class WorkflowService(
    ...
    @Transactional
    fun updateSnapshot(workflowId: UUID, snapshotId: Long, name: String, description: String?): WorkflowSnapshot {
        val trimmedName = name.trim()
        if (trimmedName.isEmpty()) {
            throw BadRequestException("Snapshot name must not be blank")
        }
        ...
        snapshots.updateNameDescription(snapshotId, trimmedName, description?.takeIf { it.isNotBlank() })
        return snapshots.findById(snapshotId)!!.toDto()
    }
