...
class WorkflowRunRepository(
    ...
    fun insertQueued(
        ...
        return jdbc.queryForObject(
            """
            insert into workflow_run (workflow_id, workflow_revision_id, status, input, start_node_id, is_debug, trigger_type)
            values (:workflowId, :workflowRevisionId, :status, :input::jsonb, :startNodeId, :isDebug, :triggerType)
            returning id
            """.trimIndent(),
            params,
            Long::class.java,
    ...
    fun findById(runId: Long): WorkflowRunRow? {
        ...
        val rows = jdbc.query(
            """
            select id, workflow_id, workflow_revision_id, status, started_at, finished_at, input, output, start_node_id, is_debug, trigger_type, created_at
            from workflow_run
            where id = :runId
            """.trimIndent(),
            params,
