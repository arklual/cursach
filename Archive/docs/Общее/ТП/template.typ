#let template(cfg: none, body) = {
    ...
    // Поля по Приложению 10 ДРИП: левое 25, правое 10, верхнее/нижнее 20
    let drip_margins = (left: 25mm, right: 10mm, top: 20mm, bottom: 20mm)
    ...
    let outline_and_normal_pages = {
        set page(
            margin: (
                ...
                left: 25mm,
                right: 10mm,
                bottom: 47mm,  // увеличено для ЕСПД-таблицы в footer
            ),
            header: [
                #set align(center)
                #set text(weight: "bold")
        ...
        // Times New Roman 12pt, 1.5 интервала, абзацный отступ 1.25 см
        set par(
            first-line-indent: (amount: 1.25cm, all: true),
            justify: true,
            leading: 1.08em,  // ≈ 1.5 межстрочный при 12pt
            spacing: 1.08em,
        )

        // Большие таблицы должны корректно разбиваться между страницами
        ...
        show heading.where(level: 2): h => block(width: 100%, above: 1.5em, below: 1.08em)[
            #set text(weight: "bold", size: 12pt)
            #par(first-line-indent: 0pt)[#counter(heading).display() #h.body]
        ]

        show heading.where(level: 3): h => block(width: 100%, above: 1.5em, below: 1.08em)[
            #set text(weight: "bold", size: 12pt)
            #par(first-line-indent: 0pt)[#counter(heading).display() #h.body]
        ]
