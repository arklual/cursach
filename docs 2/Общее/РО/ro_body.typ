...
Программа «Платформа для no-code автоматизации бизнес-процессов» предназначена для визуального проектирования, конфигурирования и автоматического исполнения бизнес-процессов (workflow) без необходимости написания программного кода со стороны конечного пользователя. Продукт обеспечивает следующие основные возможности: создание workflow из набора готовых типов узлов (HTTP-запросы, операции над потоками данных, исполнение пользовательского Python- и JavaScript-кода в безопасной песочнице, A/B-узлы для разделения трафика); настройку параметров каждого узла через панель инспектора; запуск workflow вручную, по расписанию (cron-выражение) или через входящие HTTP-запросы (webhook); мониторинг хода исполнения в реальном времени с подсветкой текущего узла; просмотр истории всех запусков с детализацией входов и выходов каждого узла; версионирование конфигурации workflow с возможностью отката к ранее сохранённым состояниям и созданием именованных snapshot-ов; сбор продуктовой аналитики и подсчёт доверительных интервалов конверсии для A/B-экспериментов.

== Эксплуатационное назначение
Программный продукт предназначен для эксплуатации тремя основными группами пользователей. *Бизнес-аналитики и продуктовые менеджеры* — основная целевая аудитория; используют платформу для автоматизации повторяющихся задач интеграции данных между системами и проведения A/B-экспериментов над пользовательским трафиком. *Системные администраторы* — отвечают за развёртывание и сопровождение платформы; для них предусмотрено индивидуальное руководство оператора серверной части (RU.17701729.09.03-04 34 03-1). *Разработчики* — используют платформу как gateway-инструмент для построения сложных конвейеров обработки данных, требующих гибкости пользовательского кода.
...
Специальной программистской квалификации для работы с платформой не требуется. Пользователь должен обладать базовыми навыками работы с веб-приложениями: использование мыши и клавиатуры, навигация по интерфейсу, заполнение форм. Для эффективного использования отдельных типов узлов рекомендуется иметь следующие знания: для HTTP-узлов — понимание основ протокола HTTP (методы GET/POST/PUT/DELETE, заголовки, тела запросов), знакомство с REST API внешних сервисов, с которыми планируется интегрироваться; для узлов потоков данных — базовое понимание операций обработки списков (filter, map, reduce); для Code Node — базовое владение языком Python или JavaScript; для настройки scheduler-триггеров — знание формата cron-выражений; для webhook-триггеров — понимание того, как настроить вызов URL в внешней системе.
...
#align(center)[_[Здесь вставить скриншот главной страницы со списком workflow]_]
...
#align(center)[_[Здесь вставить скриншот пустого холста с палитрой и инспектором]_]
...
#align(center)[_[Здесь вставить скриншот двух связанных узлов на холсте]_]
...
#align(center)[_[Здесь вставить скриншот настройки webhook-триггера со сгенерированным URL]_]
...
#align(center)[_[Здесь вставить скриншот настройки Code Node с примером Python-кода]_]
...
#align(center)[_[Здесь вставить скриншот холста с разноцветной подсветкой статусов узлов]_]
...
#align(center)[_[Здесь вставить скриншот панели истории запусков]_]
...
#align(center)[_[Здесь вставить скриншот панели версий с историей и snapshot-ами]_]
...
#figure(caption: [Сообщения пользователю интерфейса], table(columns: (auto, auto, auto), align: horizon,
